...
def build_design_ux(state: dict, matrix: list[dict], opts: dict) -> str:
    ...
    parts = [
        ...
        docs_to_read([
            ...
            ("Templates spec (PHẢI giữ cấu trúc)", "docs/architecture/ux/TEMPLATE.ux.md + TEMPLATE.design-tokens.css"),
        ...
        tasks_block([
            ...
            "**THIẾT KẾ THẲNG GIAO DIỆN BẰNG HTML — không template, không ASCII:** mỗi screen 1 file `docs/architecture/ux/mockups/{boundary}/{screen}.html` — bạn là DESIGNER, tự dựng giao diện HOÀN CHỈNH như trang web thật (app shell + nội dung thật, đẹp theo §Visual polish của skill). Luật (mockups/README.md): HTML TĨNH mở file:// xem được (không JS/build/CDN) · style CHỈ `var(--...)` link `../../design-tokens.css` (thiếu token → thêm vào design-tokens.css, không bịa tại chỗ) · nội dung thật không lorem · state phụ (loading/empty/error) = section cùng file · responsive media query · đủ :hover/:focus-visible. Gate `design_gate`: mỗi web boundary ≥1 mockup + mockup phải dùng token.",
            "Foreach FE boundary (kind web/mobile): `ux-{boundary}.md` theo TEMPLATE.ux — BEHAVIOR đi kèm mockup: user flow per FEAT Must · component states đầy đủ (default/hover/disabled/loading/error/empty) · API calls khớp `api-{be}.md` · validation FE · permission UI · a11y WCAG 2.1 AA · **§Visual polish** (app shell/spacing rhythm/type scale/primitives/interaction states/elevation — thể hiện TRONG mockup).",
