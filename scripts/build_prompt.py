...
def build_design_ux(state: dict, matrix: list[dict], opts: dict) -> str:
    ...
    parts = [
        ...
        docs_to_read([
            ...
            ("Templates (PHẢI giữ cấu trúc)", "docs/architecture/ux/TEMPLATE.ux.md + TEMPLATE.design-tokens.css + mockups/TEMPLATE.mockup.html"),
        ...
        tasks_block([
            ...
            "**DESIGN BẰNG HTML — không ASCII wireframe:** mỗi screen 1 mockup `docs/architecture/ux/mockups/{boundary}/{screen}.html` theo `mockups/TEMPLATE.mockup.html` — HTML TĨNH (mở file:// xem được, không JS/build/CDN), style CHỈ `var(--...)` link `../../design-tokens.css`, nội dung THẬT (text/số liệu mẫu, không lorem), state phụ (loading/empty/error) = section trong cùng file, responsive media query. Gate `design_gate`: mỗi web boundary ≥1 mockup + mockup phải dùng token.",
