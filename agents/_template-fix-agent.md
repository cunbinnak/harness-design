...
> Fix sub-agent. **Mode A** spawn bởi MAIN qua `/fix-bugs` (MANUAL_TEST) hoặc test-execute-chain. **Mode B** spawn bởi MAIN trong `/review-dev`. Fix KHÔNG tự gọi review-agent (sub-agent không nest spawn).
...
- **Primary** (invoke ngay): `rules-{{kind}}`, `bug-logging` (Mode A) — Mode B chỉ `rules-{{kind}}`
