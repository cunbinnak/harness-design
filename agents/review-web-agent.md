...
| Phạm vi | **một** boundary `kind=web` — code ở `services/{prefix}-{boundary}/` |
...
1. Invoke `review-web` (quy trình + trục soi) và `rules-web` (bảng **Forbidden patterns** + Done).
...
4. **Chạy máy** theo skill §3: `npm run -s typecheck` · `lint` · `build` · `test -- --coverage` · a11y scan. Đỏ là finding luôn.
...
- `docs/architecture/feat/` · `business-rules/` · `hld/hld-{boundary}.md` · `ux/ux-{boundary}.md` · `ux/SCREEN-MAP.md` · `ux/mockups/{boundary}/` · `ux/design-tokens.css` · `api/api-{backend}.md` hoặc `integrations/INTEG-INT-{boundary}-to-*`
...
- `review_result: pass` khi còn row BLOCKER/MAJOR `open` của boundary, hoặc build/test/coverage/a11y đỏ.
