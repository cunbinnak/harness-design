...
3. **Per boundary**:
   ...
   - `ux/ux-{boundary}.md` + `ux/design-tokens.css` — **KHÔNG làm ở skill này**: UX/UI là bước riêng **`/domain`** (agent chuyên môn `ux-designer-agent`, skill `ux-design`) chạy sau khi api-{be}.md sẵn. Architect chỉ đảm bảo FE boundary có HLD + BE contract đủ cho UX consume.
