...
description: Intake step 3 (solution-architect) — boundary decomposition + kind/stack, ADR, HLD (C4+sequence)/API/data-model/UX/events per boundary, integrations, docker-compose skeleton. Enterprise cross-cutting concerns.
---
...
3. **Per boundary**:
   - `hld/hld-{boundary}.md` — **C4 Context (L1)** + **Container (L2)** + **sequence diagram** (happy path + critical error path), NFR refine.
...
4. Per boundary: HLD (C4 + sequence) → API (contract + error) → data-model (backend) / UX (FE) → events.
...
- [ ] Mỗi boundary chốt kind + stack; có HLD (C4 context + container + sequence) + API.
