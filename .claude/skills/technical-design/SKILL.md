...
description: Stage DESIGN (/design, solution-architect) — boundary decomposition + kind/stack, ADR, HLD (theo TEMPLATE.hld ZIP)/API/data-model/events per boundary, integrations, docker-compose skeleton. UX/UI KHÔNG thuộc skill này (lượt riêng của /design, ux-designer-agent — chỉ chạy khi kind là web/mobile). Sau DOMAIN, trước PLAN. Enterprise cross-cutting concerns.
---

# Technical Design Skill

## Khi load
Command **`/design`** (stage **DESIGN** → PLAN) — agent `solution-architect-agent`, sau `/domain`.
...
3. **Per boundary**:
   ...
   - `ux/ux-{boundary}.md` + `ux/design-tokens.css` — **KHÔNG làm ở skill này**: UX/UI là bước riêng **`/design`** (agent chuyên môn `ux-designer-agent`, skill `ux-design`) chạy sau khi api-{be}.md sẵn. Architect chỉ đảm bảo FE boundary có HLD + BE contract đủ cho UX consume.
...
4. Per boundary: HLD (theo `TEMPLATE.hld`) → API (contract + error) → data-model (backend) → events. (FE boundary: HLD ở đây; UX = `/design` sau khi api sẵn.)
...
- [ ] Backend boundary có data-model **theo TEMPLATE.data-model** (mỗi bảng có mục đích; no FK — liên kết qua id; state machine cho entity có status). (UX cho FE boundary: `/design` tự làm khi kind là web/mobile — gate `/design --end` vẫn đòi đủ.)
...
- [ ] **Trả nợ TODO-engineer (gate `todo_resolved` @/design):** mọi `TODO engineer` / `TBD (DESIGN)` translator để lại trong eng feat/BR đã ĐIỀN — BR `enforcement_location` chỉ đúng nơi enforce (api/data-model/event handler), FEAT `consumes_contracts` trỏ contract thật. Chưa chốt → Open question có chủ, KHÔNG để TBD.
