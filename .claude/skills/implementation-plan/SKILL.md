...
5. **Materialize MATRIX** (mỗi boundary: `wave` + `features[]` + `depends_on`) + **KG skeleton** per boundary.
...
- [ ] MATRIX mỗi boundary đủ `kind/prefix/tech/wave/features/depends_on`; KG skeleton mọi boundary.
