...
- `BUG` id tăng dần `BUG-NNN`; `status`: `open|in_progress|fixed|closed|wontfix`; `origin`: `auto|manual`; `sev`: `high|medium|low`.
...
2. `origin`: `auto` từ test-execute, `manual` từ UAT (MANUAL_TEST).
