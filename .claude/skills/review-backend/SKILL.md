...
> **Layer/package/cycle giờ có ArchUnit chặn máy** (`ref-backend-pattern §7.5`, gate `code_compliance` đòi test tồn tại; `gradle test` chạy nó). Review-agent tập trung phần **semantic/borderline** ArchUnit không bắt được (naming ý nghĩa, class lạc chỗ tinh vi, dead-package). Xác nhận `ArchitectureTest.java` tồn tại + rule khớp layout; thiếu/rỗng rule = BLOCKER.
...
- [ ] **`ref-backend-logging`**: logger qua **`@Slf4j`** (KHÔNG `LoggerFactory.getLogger` thủ công); SLF4J structured (JSON) + **MDC** correlation (traceId/tenantId) + **mask PII** + level đúng (ERROR/WARN/INFO/DEBUG).
...
- [ ] **Schema-drift (BLOCKER — chống lộ ở handoff):** có ≥1 integration test **BOOT Spring context trên Testcontainers Postgres + migration + `ddl-auto: validate`**; **entity ↔ migration nhất quán** (tên cột, kiểu: `varchar(n)`↔`CHAR(n)`, `TIMESTAMPTZ`↔`Instant`, nullable). Thiếu test boot-context hoặc lệch entity↔migration → BLOCKER (đây là lỗi escape dev+review tới `/run-wave` connect DB mới lộ).
...
Ràng buộc phải ở **DB (unique/FK/check)** hoặc **kiểm ở service**.

**3. Phân quyền — chỗ hay thủng nhất.**
```bash
grep -rn "findById\|getById\|findOne\|@Query" --include=*.java src/
```
