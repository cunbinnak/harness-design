...
| Hợp đồng surface đã giao (cross-wave) | `tracking/BC-LEDGER.md` §1 tích luỹ · §3 re-arm mỗi wave |
| Đối kháng trước khi code | `tracking/challenge-log.md` (lọc theo wave) |
| Quyết định khi mơ hồ | `tracking/decisions.md` (`py scripts/decide.py`) |
| Snapshot + hợp đồng wave đã đóng | `archive/wave-{N}/` (toàn bộ docs+tracking) · `DELIVERED.md` |
...
BOOTSTRAP → DISC_D0 → DISC_D1 → DISC_D2 → DISC_D3 → DOMAIN_AUTHORING → DESIGN → PLAN → REVIEW
   ...
   DISC_D3 → /discover → DOMAIN                   │      /domain↻ /domain↻  /domain   review/approve
   /domain (author business, self-loop) ─┘            /domain (ký) → /domain (dịch eng) → /domain
```
...
```json
{
  ...
  "deferred": [{"item": "...", "reason": "...", "tracked_in": "wave-{N+1} | RF-NNN"}],
...
- Review: `review_result`, `no_open_findings`, `coverage_pct`
...
```bash
py scripts/gates.py --list      # gate của TỪNG command, in thẳng từ GATE_RULES
```

Bảng gate **không được chép vào tài liệu**. Bản chép trôi ngay lần đổi gate kế, và tài liệu trôi
tệ hơn tài liệu thiếu — người đọc tin nó. (Bảng cũ ở đây từng thiếu 6 gate và còn liệt kê lệnh
đã xoá.)

Mọi content-gate force-bypass được (`force:true,reason` → audit `tracking/decisions.md`).
...
- **Vá lỗ hổng:** `PreToolUse(SlashCommand)` chặn MAIN tự chạy slash-command ∈ `GATE_RULES` (nếu không, invoke sẽ fire lại `UserPromptSubmit` → reset cờ → lệnh kế lọt). User **gõ tay** slash-command = pre-loaded, MAIN không gọi tool → không ảnh hưởng. **`Skill` tool KHÔNG chặn** — sub-agent CẦN load skill convention của chính nó (domain-po/run-wave/ux-design…); Skill không transition state nên không phải vector tự-nối-lệnh.
