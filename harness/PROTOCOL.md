...
| `design-ux` | (self-loop refine UX/UI cho FE boundary — ux-designer-agent, skill ux-design; **mockup HTML tĩnh** `ux/mockups/{b}/*.html` + ux-*.md + design-tokens.css. KHÔNG gate, KHÔNG advance) |
| `design-end` | `design_gate` (ADR≥3 + INTEG≥1 + per-boundary completeness: backend/bff→hld+api, web/mobile→hld+ux + design-tokens.css khi có web + **mỗi web boundary ≥1 mockup HTML dùng token**) + `todo_resolved` (marker `TODO engineer`/`TBD (DESIGN)` trong eng feat/BR đã điền hết) |
