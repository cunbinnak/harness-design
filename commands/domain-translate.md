...
1. `py scripts/harness.py domain-translate complete` **chỉ chạy SAU khi dịch xong** — nhưng STATE giữ DOMAIN_AUTHORING (self-loop), gate `domain_signed` verify mọi business doc `approved:true` TRƯỚC.
...
## Gate `domain_signed`
MỌI business doc ở `docs/domain/` phải `approved: true` (ký TRƯỚC, dịch SAU). Còn doc chưa ký → chặn, về `/domain-approve`. Override: `'{"force":true,"reason":"..."}'`.
