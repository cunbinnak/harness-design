...
| "Chỗ nào tắc cứng, ai đang chờ gì?" | `tracking/blockers.md` — chỗ dừng hợp lệ THỨ BA (khác mơ hồ→`decide.py`, khác ngoài-scope→wave sau). Ghi xong **đi làm việc khác ngay**; cột `Đã thử gì` trống thì chưa phải blocker. Không reset khi mở wave — `/next-wave` đếm và nhắc |
